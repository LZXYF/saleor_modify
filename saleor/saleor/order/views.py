...
from ..alipay.views import payOrder
...
def start_payment(request, order, gateway):
    ...
    with transaction.atomic():
        print("YYYYYYYYYYYYYYYYYYYYYYYYYY-----执行了",++i)
        ...
        print(payment.billing_last_name)
        if form.is_valid():
            try:
                print("验证完成的    的fromtoken:::" , form.get_payment_token())
                request.session['total'] = str(Decimal(payment.total).quantize(Decimal('0.00')))
                request.session['order_id'] = str(order.id)
                request.session['token'] = order.token
                # return redirect("/alipay/alipay/")
                return TemplateResponse(request, "order/choosePayWays.html", {})
    ...
    print("payment是什么：：" , ctx['payment'].total)
...
def checkout_success(request, token):
    ...
    if request.user.is_authenticated:
        print("执行了iffffffffffffffffffffffffffffffffffffffffffff")
        return TemplateResponse(request, "order/checkout_success.html", ctx)
    form_data = request.POST.copy()
    print("这个玩意是什么？？？？",form_data)
    if form_data:
